...
Wine::Wine(const char *l, int y, const int yr[], const int bot[])
{
    ...
    years = y;
    ArrayInt t_yr(yr, y);
    ArrayInt t_bot(bot, y);
    yearsAndNum = PairArray(ArrayInt(yr, y), ArrayInt(bot, y));
}
...
Wine::Wine(const char *l, int y)
{
    label = l;
    years = y;
    yearsAndNum = PairArray(ArrayInt(y), ArrayInt(y));
}
...
void Wine::GetBottles()
{
    ...
    {
        cout << "Enter year: ";
        cin >> yearsAndNum.first()[i];
        cout << "Enter bottles for that year: ";
        cin >> yearsAndNum.second()[i];
    }
    cin.get();
}
...
{
    int sum;
    ArrayInt temp = yearsAndNum.second(); // это сделано через временный объект, поскольку получить доступ к yearsAndNum.second[i] невозможно без серьёзной модификации Pair
    for (int i = 0; i < years; i++)
       sum += temp[i];
    return sum;
}
...
void Wine::Show()
{
    ...
    for (int i = 0; i < years; i++)
        cout << '\t' << yearsAndNum.first()[i] << '\t' << yearsAndNum.second()[i] << endl;
}
